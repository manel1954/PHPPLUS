...
<title>Panel PHPPLUS - EA1HG</title>
...
/* 1. Banderas más grandes */
.flag-svg { display: inline-flex; align-items: center; justify-content: center; vertical-align: middle; line-height: 0; margin-right: 6px; }
.flag-svg svg { display: block; width: 42px; height: 28px; border-radius: 3px; box-shadow: 0 0 4px rgba(0,0,0,.5); }

/* 2. Texto de InfoBar más pequeño y compacto */
...
.nx-info-val { 
    font-family: var(--font-mono); font-size: 0.6rem; /* Reducido de 0.72 a 0.6 */
    font-weight: bold; text-align: center; line-height: 1.1; 
    display: block; width: 100%; 
    white-space: normal; overflow-wrap: anywhere; word-break: break-word; /* Rompe líneas si es necesario */
}

/* 3. Media queries para ajustes en pantallas muy pequeñas */
@media (max-width: 600px) {
    ...
    .flag-svg svg { width: 32px; height: 21px; }
}

/* Ajustes generales para evitar roturas de flexbox */
...
.lh-body::-webkit-scrollbar, .log-output::-webkit-scrollbar, .fedit-area::-webkit-scrollbar, 
.update-console::-webkit-scrollbar, .xterm-out::-webkit-scrollbar { width: 6px; height: 6px; }
.lh-body::-webkit-scrollbar-thumb, .log-output::-webkit-scrollbar-thumb, 
.fedit-area::-webkit-scrollbar-thumb { background: var(--border); border-radius: 3px; }
.lh-body, .log-output, .fedit-area, .update-console, .xterm-out { 
    scrollbar-width: thin; scrollbar-color: var(--border) transparent; 
}

/* Animaciones vendor-prefix */
...
@-webkit-keyframes spin { to { -webkit-transform: translateY(-50%) rotate(360deg); transform: translateY(-50%) rotate(360deg); } }
@keyframes spin { to { -webkit-transform: translateY(-50%) rotate(360deg); transform: translateY(-50%) rotate(360deg); } }
...
.sw-knob { position: absolute; top: 3px; left: 3px; width: 20px; height: 20px; background: #e95c04; border-radius: 2px; box-shadow: 0 1px 4px rgba(0,0,0,.5); transition: -webkit-transform .3s cubic-bezier(.4,0,.2,1); transition: transform .3s cubic-bezier(.4,0,.2,1); transition: transform .3s cubic-bezier(.4,0,.2,1), -webkit-transform .3s cubic-bezier(.4,0,.2,1); }
.sw input:checked ~ .sw-knob { -webkit-transform: translateX(28px); transform: translateX(28px); background: var(--green); box-shadow: 0 0 8px rgba(0,255,159,.6); }
...
.nx-txbar.active { background: linear-gradient(90deg,transparent,var(--green),transparent); background-size: 200% 100%; -webkit-animation: scan 1.4s linear infinite; animation: scan 1.4s linear infinite; }
.nx-txbar.active-ysf { background: linear-gradient(90deg,transparent,var(--violet),transparent); background-size: 200% 100%; -webkit-animation: scan 1.4s linear infinite; animation: scan 1.4s linear infinite; }
.nx-txbar.active-nxdn { background: linear-gradient(90deg,transparent,#ffd700,transparent); background-size: 200% 100%; -webkit-animation: scan 1.4s linear infinite; animation: scan 1.4s linear infinite; }
.nx-txbar.dstar-tx { background: linear-gradient(90deg,transparent,#00e5ff,transparent); background-size: 200% 100%; -webkit-animation: scan 1.4s linear infinite; animation: scan 1.4s linear infinite; }
...
.lh-header { background: #1c1c24; border-bottom: 1px solid var(--border); padding: .4rem 1rem; display: grid; grid-template-columns: 1.2fr 1.6fr .7fr .7fr .5fr; gap: .3rem; font-family: var(--font-mono); font-size: .6rem; color: var(--text-dim); letter-spacing: .1em; text-transform: uppercase; }
.lh-body { flex: 1; overflow-y: auto; }
.lh-row { display: grid; grid-template-columns: 1.2fr 1.6fr .7fr .7fr .5fr; gap: .3rem; padding: .45rem 1rem; border-bottom: 1px solid rgba(30,45,61,.6); align-items: center; transition: background .2s; }
...
.ysf .lh-panel { border-color: #2d1a4a; }
.ysf .lh-header { background: #1a1424; border-bottom-color: #2d1a4a; color: #4a2a7a; }
.ysf .lh-call { color: var(--violet); }
.ysf .lh-tx-dot { background: var(--violet); box-shadow: 0 0 6px var(--violet); }
.ysf .lh-row:hover { background: rgba(181,122,255,.04); }
.ysf .lh-row.lh-active { background: rgba(181,122,255,.08); }

.nxdn .lh-panel { border-color: #4a4a00; }
.nxdn .lh-header { background: #1a1a0a; border-bottom-color: #4a4a00; color: #707000; }
.nxdn .lh-call { color: #ffd700; }
.nxdn .lh-tx-dot { background: #ffd700; box-shadow: 0 0 6px #ffd700; }
...
.dropdown-menu-custom { display: none; position: absolute; top: 100%; left: 50%; -webkit-transform: translateX(-50%); transform: translateX(-50%); background: var(--surface); border: 1px solid var(--border); border-radius: 6px; min-width: 270px; z-index: 1000; box-shadow: 0 8px 24px rgba(0,0,0,.5); overflow: hidden; padding-top: .4rem; }
...
<script>
// ✅ BANDERAS SVG MÁS GRANDES (42x28)
const FLAGS = {
  ES: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 750 500" width="42" height="28"><rect width="750" height="500" fill="#c60b1e"/><rect y="125" width="750" height="250" fill="#ffc400"/></svg>',
  PT: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 660 400" width="42" height="28"><rect width="660" height="400" fill="#006600"/><rect x="330" width="330" height="400" fill="#ff0000"/><circle cx="330" cy="200" r="70" fill="#ffcd00"/><circle cx="330" cy="200" r="52" fill="#006600"/></svg>',
  FR: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#fff"/><rect width="300" height="600" fill="#002395"/><rect x="600" width="300" height="600" fill="#ed2939"/></svg>',
  IT: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1500 1000" width="42" height="28"><rect width="500" height="1000" fill="#009246"/><rect x="500" width="500" height="1000" fill="#fff"/><rect x="1000" width="500" height="1000" fill="#ce2b37"/></svg>',
  GB: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 30" width="42" height="21"><rect width="60" height="30" fill="#00247d"/><path d="M0 0L60 30M60 0L0 30" stroke="#fff" stroke-width="3.5"/><path d="M0 0L60 30M60 0L0 30" stroke="#cf142b" stroke-width="1.8"/><path d="M30 0v30M0 15h60" stroke="#fff" stroke-width="5"/><path d="M30 0v30M0 15h60" stroke="#cf142b" stroke-width="2.5"/></svg>',
  DE: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 600" width="42" height="28"><rect width="1000" height="600"/><rect y="200" width="1000" height="200" fill="#dd0000"/><rect y="400" width="1000" height="200" fill="#ffce00"/></svg>',
  US: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1235 650" width="42" height="22"><rect width="1235" height="650" fill="#b22234"/><rect width="1235" height="50" fill="#fff" y="50"/><rect width="1235" height="50" fill="#fff" y="150"/><rect width="1235" height="50" fill="#fff" y="250"/><rect width="1235" height="50" fill="#fff" y="350"/><rect width="1235" height="50" fill="#fff" y="450"/><rect width="1235" height="50" fill="#fff" y="550"/><rect width="494" height="350" fill="#3c3b6e"/></svg>',
  CA: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 250" width="42" height="21"><rect width="500" height="250" fill="#fff"/><rect width="125" height="250" fill="#ff0000"/><rect x="375" width="125" height="250" fill="#ff0000"/></svg>',
  BR: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1100 700" width="42" height="28"><rect width="1100" height="700" fill="#009739"/><path d="M550 80L1020 350L550 620L80 350Z" fill="#fedd00"/><circle cx="550" cy="350" r="140" fill="#002776"/></svg>',
  AR: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#fff"/><rect width="900" height="200" fill="#75aadb"/><rect y="400" width="900" height="200" fill="#75aadb"/></svg>',
  JP: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#fff"/><circle cx="450" cy="300" r="160" fill="#bc002d"/></svg>',
  AU: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 600" width="42" height="21"><rect width="1200" height="600" fill="#00008b"/><rect width="600" height="300" fill="#fff"/><path d="M0 0L600 300M600 0L0 300" stroke="#e4002b" stroke-width="4"/><path d="M300 0v300M0 150h600" stroke="#e4002b" stroke-width="6"/></svg>',
  ZA: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#007749"/><path d="M0 0L300 300L0 600Z" fill="#000"/><path d="M0 0L300 300L0 600Z" stroke="#ffb81c" stroke-width="25" fill="none"/><path d="M300 300H900" stroke="#fff" stroke-width="60"/><path d="M300 300H900" stroke="#000" stroke-width="30"/></svg>',
  FI: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1800 1100" width="42" height="28"><rect width="1800" height="1100" fill="#fff"/><rect y="400" width="1800" height="300" fill="#003580"/><rect x="500" width="300" height="1100" fill="#003580"/></svg>',
  NL: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#ae1c28"/><rect y="200" width="900" height="200" fill="#fff"/><rect y="400" width="900" height="200" fill="#21468b"/></svg>',
  CH: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 480 480" width="42" height="42"><rect width="480" height="480" fill="#ff0000"/><rect x="200" y="120" width="80" height="240" fill="#fff"/><rect x="120" y="200" width="240" height="80" fill="#fff"/></svg>',
  AT: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#ed2939"/><rect y="200" width="900" height="200" fill="#fff"/><rect y="400" width="900" height="200" fill="#ed2939"/></svg>',
  PL: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 800" width="42" height="28"><rect width="1280" height="800" fill="#dc143c"/><rect width="1280" height="400" fill="#fff"/></svg>',
  RU: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="42" height="28"><rect width="900" height="600" fill="#fff"/><rect y="200" width="900" height="200" fill="#0039a6"/><rect y="400" width="900" height="200" fill="#d52b1e"/></svg>',
  GR: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 27 18" width="42" height="28"><rect width="27" height="18" fill="#0d5eaf"/><path d="M9 0v9h9M0 9h27M0 13.5h27M0 4.5h27" stroke="#fff" stroke-width="2"/></svg>',
  LT: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 400" width="42" height="28"><rect width="600" height="400" fill="#fdb913"/><rect y="133" width="600" height="134" fill="#006a44"/><rect y="267" width="600" height="133" fill="#c1272d"/></svg>',
  HR: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 600" width="42" height="21"><rect width="1200" height="600" fill="#ff0000"/><rect y="200" width="1200" height="200" fill="#fff"/><rect y="400" width="1200" height="200" fill="#171796"/></svg>'
};

function getFlagByCall(callsign) {
    if (!callsign) return '<span class="flag-svg">'+FLAGS.ES+'</span>';
    const cs = callsign.toUpperCase().trim();
    const prefixes = [
        {re:/^EA[0-9]|EB|EC|ED|EE|EF|EG|EH/, code:'ES'}, {re:/^CT|CU|CV|CQ/, code:'PT'},
        {re:/^F[A-Z]|FT[0-9A-Z]|FM|FO|FH|FJ|FK|FL|FP|FR|FS/, code:'FR'}, {re:/^I[0-9]|IK|IW|IZ/, code:'IT'},
        {re:/^G[0-9]|M[0-9]|2E[0-9]|2[0-9]|GB|MJ|MU/, code:'GB'}, {re:/^D[ALM]|DA|DB|DC|DD|DE|DF|DG|DH|DI|DJ|DK|DL|DM|DN|DO|DP|DQ|DR/, code:'DE'},
        {re:/^K[0-9]|W[0-9]|N[0-9]|AA|AB|AC|AD|AE|AF/, code:'US'}, {re:/^VE[0-9]|VA[0-9]|VO[0-9]|VY[0-9]/, code:'CA'},
        {re:/^PY[0-9]|PU|PV|PW|PX/, code:'BR'}, {re:/^LU[0-9]|LV|LW|LX/, code:'AR'},
        {re:/^JA[0-9]|JB|JC|JD|JE|JF|JG|JH|JI|JJ|JK|JL|JM|JN|JO|JP|JQ|JR|JS|JT|JU|JV|JW|JX|JY|JZ/, code:'JP'},
        {re:/^VK[0-9]|VL|VM|VN|VO|VP|VQ|VR|VS|VT|VU|VV|VW|VX|VY|VZ/, code:'AU'}, {re:/^ZS[0-9]|ZT|ZU|ZV|ZW|ZX|ZY|ZZ/, code:'ZA'},
        {re:/^OH[0-9]|OG|OI|OJ|OK|OL|OM|ON|OO|OP|OQ|OR|OS|OT|OU|OV|OW|OX|OY|OZ/, code:'FI'},
        {re:/^PA[0-9]|PB|PC|PD|PE|PF|PG|PH|PI|PJ|PK|PL|PM|PN|PO|PP|PQ|PR|PS|PT|PU|PV|PW|PX|PY|PZ/, code:'NL'},
        {re:/^HB[0-9]|HB9/, code:'CH'}, {re:/^OE[0-9]/, code:'AT'}, {re:/^SP[0-9]|SQ|SR/, code:'PL'},
        {re:/^UA[0-9]|UB|UC|UD|UE|UF|UG|UH|UI|UJ|UK|UL|UM|UN|UO|UP|UQ|UR|US|UT|UU|UV|UW|UX|UY|UZ/, code:'RU'},
        {re:/^SV[0-9]|SW|SX|SY|SZ/, code:'GR'}, {re:/^LY[0-9]|LZ/, code:'LT'}, {re:/^9A[0-9]/, code:'HR'}
    ];
    for(const p of prefixes){ if(p.re.test(cs)) return '<span class="flag-svg">'+FLAGS[p.code]+'</span>'; }
    return '<span class="flag-svg">'+FLAGS.ES+'</span>';
}
...
const DMR_IDLE_TIMEOUT=2000,YSF_IDLE_TIMEOUT=2000,NXDN_IDLE_TIMEOUT=2000;
...
// ⏱️ LÓGICA TIEMPO REAL
async function fetchTransmission(){try{const r=await fetch('?action=transmission');const d=await r.json();if(d.active){dmrLastActiveTs=Date.now();showActive(d);}else{if(currentlyActive)showIdle();}renderLastHeard(d.lastHeard||[],d.active?d.callsign:null);}catch(e){if(currentlyActive)showIdle();}}
async function fetchYSFTransmission(){try{const r=await fetch('?action=ysf-transmission');const d=await r.json();if(d.active){ysfLastActiveTs=Date.now();showYSFActive(d);}else{if(ysfCurrentlyActive)showYSFIdle();}renderYSFLastHeard(d.lastHeard||[],d.active?d.callsign:null);}catch(e){if(ysfCurrentlyActive)showYSFIdle();}}
...
function showDStarActive(d){dstarCurrentlyActive=true;animateDStarVU(true);document.getElementById('dstarTxBar').className='nx-txbar';document.getElementById('dstarTxBar').style.background='linear-gradient(90deg,transparent,#00e5ff,transparent)';document.getElementById('dstarTxBar').style.backgroundSize='200% 100%';document.getElementById('dstarTxBar').style.webkitAnimation='scan 1.4s linear infinite';document.getElementById('dstarTxBar').style.animation='scan 1.4s linear infinite';const src=document.getElementById('dstarSource');if(d.source==='RF'){src.textContent='RF';src.className='nx-source rf';}else{src.textContent='NET';src.className='nx-source net';}const flag=getFlagByCall(d.callsign.replace(/\/.*$/,''));document.getElementById('dstarNxCenter').innerHTML=`<div class="nx-callsign dstar">${flag} ${esc(d.callsign)}</div>`+(d.name?`<div class="nx-name dstar">${esc(d.name)}</div>`:'');}
...
// ⏱️ LÓGICA TIEMPO REAL
async function fetchDStarTransmission(){try{const r=await fetch('?action=dstar-transmission');const d=await r.json();if(d.active)showDStarActive(d);else{if(dstarCurrentlyActive)showDStarIdle();}renderDStarLastHeard(d.lastHeard||[],d.active?d.callsign:null);}catch(e){if(dstarCurrentlyActive)showDStarIdle();}}
